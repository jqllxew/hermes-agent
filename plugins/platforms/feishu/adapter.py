...
class FeishuAdapter(BasePlatformAdapter):
    ...
    async def _process_inbound_message(
        ...
        *,
        ...
        if chat_type == "group":
            ...
            # Only overwrite text when context fetch succeeds — a None return
            # (API failure/timeout) must not blank the original trigger text.
            context_text = await self._fetch_chat_context_sync(chat_id=chat_id, trigger_message_id=message_id)
            if context_text:
                text = context_text
    ...
    async def _get_display_name(self, open_id: str) -> str:
        ...
        Local optimization: cache → known_bot_names(config) → fallback open_id.
        Group member names are pre-warmed by _populate_chat_member_names, so no
        contact API scope is needed here.
        """
        if not open_id:
            return "Unknown"
